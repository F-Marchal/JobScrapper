...
class LogicalWrapper(ClauseElementWrapper):
    ...
    def __init__(
            self,
            op: Callable[[ClauseElement | bool, ClauseElement | bool], ClauseElement],
            help_: str,
            symbols: list[str],
    ):
        """
    ...
    @property
    def op(self) -> Callable[[ClauseElement | bool, ClauseElement | bool], ClauseElement]:
        return self._op

    @op.setter
    def op(self, operator:  Callable[[ClauseElement | bool, ClauseElement | bool], ClauseElement]):
        self._op = operator
...
XOR_WRAPPER = LogicalWrapper(
    op=lambda first, second: and_(or_(first, second), not_(and_(first, second))),
...
STRING_TO_LOGICAL_WRAPPERS: dict[str, LogicalWrapper] = {s: cw for cw in LOGICAL_WRAPPERS for s in cw.symbols}
