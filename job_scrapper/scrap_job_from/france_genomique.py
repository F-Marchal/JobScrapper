...
class FranceGenomiqueScrapper(srk.JobScrapperSkeleton):
    ...
    def complete_job_page_parsing(
        ...
        for cells in soup.find_all( # pylint: disable=W0612
            ...
            str_cell = str(cells)
            tmp_contract = cells.find("a", class_="style-accent-bg tmb-term-evidence font-ui")
            if tmp_contract:
                contract_type = tmp_contract.get_text(strip=True)
            else:
                contract_type = None

            tmp_title_localisation = cells.find("h2", class_="t-entry-title h6 title-scale").get_text(strip=True)
            title, localisation = tmp_title_localisation.split("(")
            localisation = localisation.removesuffix(")")
            url = re.findall('<a class="btn btn-link" href="(.*)" ', str_cell)[0]
