...
class TestGeolocalisation(BaseGeoTest):
    ...
    def test_get_localisation_from_database(self):
        ...
        with Places.get_session(self.get_db_path(), logger=self.icl.logger) as session:
            result = Geolocalisation.get_localisation_from_database(session, "Paris, France")
    ...
    def test_get_distances_from_database(self):
        """Test get_localisation_from_database"""
        with Places.get_session(self.get_db_path(), logger=self.icl.logger) as session:
            result = Geolocalisation.get_distances_from_database(session, "Paris, France", "Knokke-le-Zoute, Belgium")
            assert result is None # The database should be empty
    ...
    def test_compute_distance__invalid_place_obj(self, entry1: Places, entry2: Places):
        geo = self.make_geo()
