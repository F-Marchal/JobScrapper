...
class BaseTest:
    ...
    def _setup_tempdir(self, request):
        ...
        test_dir_name_in_progress = f"IN_PROGRESS-{type(self).__name__}-{test_name}-{start_date}"
        test_dir_name_succeed = f"SUCCEED-{type(self).__name__}-{test_name}-{start_date}"
        ...
        if rep_call is not None and rep_call.failed:
            os.rename(self.test_folder, os.path.join(test_abs_parent_folder, test_dir_name_failed))
        elif keep_dir:
            os.rename(self.test_folder, os.path.join(test_abs_parent_folder, test_dir_name_succeed))
        else:
...
class LogFile:
    ...
    class ScreenVar:
        ...
        """
        def __init__(self, opened_file, name, obj):
            self.name = name
            self.obj = obj
        ...
        def _screen(self):
            ...
            if isinstance(self.obj, (str, list, dict, set, tuple)):
                ...
                self.content = self.obj.__dict__ if "__dict__" in obj_attr else  self.obj.__repr__
    ...
    def __init__(self, file_path: str):
        ...
        if os.path.exists(file_path):
            raise FileExistsError(f"Can not write log in '{file_path}' (file exist)")
        if not os.path.isdir(os.path.dirname(file_path)):
            raise FileExistsError(f"Can not write log in '{file_path}' (unknown folder)")
