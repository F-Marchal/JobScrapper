...
def ilike_filter(items_list: list[str], pattern: str, inverse=False) -> list[str]:
    ...
    if inverse:
        return [item for item in items_list if not re.search(regex, item, re.IGNORECASE)]
    else:
        return [item for item in items_list if re.search(regex, item, re.IGNORECASE)]
...
class TestComparisonWrapper(BaseTest):
    ...
    def test_ensure_all_operand_are_tested(self):
        ...
        assert len(self.test_operands_simple_parameters) == 2
        print()
        for vals in self.test_operands_simple_parameters[1]:
            tested_op.add(vals[1])

        for vals in self.test_operands_complex_parameters[1]:
            tested_op.add(vals[1])
    ...
    test_operands_simple_parameters = [ # IA Generated
        ...
        [
            ...
            # ====== DATETIME ======
            (datetime(2025, 1, 1), "==", datetime(2025, 1, 1), lambda a, b: a == b),
            (datetime(2025, 1, 1), "==", datetime(2024, 12, 31), lambda a, b: a == b),  # False
            (datetime(2025, 1, 1), "!=", datetime(2024, 12, 31), lambda a, b: a != b),
            (datetime(2025, 1, 1), "!=", datetime(2025, 1, 1), lambda a, b: a != b),  # False
            (datetime(2025, 1, 2), ">", datetime(2025, 1, 1), lambda a, b: a > b),
            (datetime(2025, 1, 1), ">", datetime(2025, 1, 2), lambda a, b: a > b),  # False
            (datetime(2025, 1, 1), ">=", datetime(2025, 1, 1), lambda a, b: a >= b),
            (datetime(2024, 12, 31), ">=", datetime(2025, 1, 1), lambda a, b: a >= b),  # False
            (datetime(2024, 12, 31), "<", datetime(2025, 1, 1), lambda a, b: a < b),
            (datetime(2025, 1, 1), "<", datetime(2024, 12, 31), lambda a, b: a < b),  # False
            (datetime(2025, 1, 1), "<=", datetime(2025, 1, 1), lambda a, b: a <= b),
            (datetime(2025, 1, 2), "<=", datetime(2025, 1, 1), lambda a, b: a <= b),  # False
        ]
    ]
    @pytest.mark.parametrize(
        *test_operands_simple_parameters
    )
    def test_simple_operands(self, val1: Any, operand: str, val2: Any, true_operand: Callable[[Any, Any], bool]):
        """
    ...
    test_operands_complex_parameters =[ # IA Generated
        "column, operand, val2, true_operand",
        # ====== STRING Only ======
        [
            (StringTable.name, "<-", "%lo%", lambda entries, pat: ilike_filter(entries, pat)),
            (StringTable.name, "<-", "%bye%", lambda entries, pat: ilike_filter(entries, pat)),
            (StringTable.name, "Contains", "%foo%", lambda entries, pat: ilike_filter(entries, pat)),
            (StringTable.name, "Contains", "%world", lambda entries, pat: ilike_filter(entries, pat)),

            (StringTable.name, "x-", "%bye%", lambda entries, pat: ilike_filter(entries, pat, True)),
            (StringTable.name, "x-", "%lo%", lambda entries, pat: ilike_filter(entries, pat, True)),
            (StringTable.name, "~Contains", "%world", lambda entries, pat: ilike_filter(entries, pat, True)),
            (StringTable.name, "~Contains", "%foo%", lambda entries, pat: ilike_filter(entries, pat, True)),
        ],
    ]
    @pytest.mark.parametrize(
        *test_operands_complex_parameters
    )
    def test_complex_operands(self, column: Column, operand: str, val2: Any, true_operand: Callable[[Any, Any], list]):
        ...
        with StringTable.get_session(os.path.join(self.test_folder, "test_database.db")) as session:
            all_names = session.execute(select(column)).scalars().all()
            self.screen_var("All_names", all_names)
            result = session.execute(select(column).where(wrapper(column, val2))).scalars().all()
